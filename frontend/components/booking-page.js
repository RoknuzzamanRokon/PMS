...
export function BookingPage({ propertyId }) {
  ...
  const selectedPlan = useMemo(
    () => availablePlans.find((plan) => plan.rate_id === selectedRateId) || null,
    [availablePlans, selectedRateId],
  );
  ...
  const estimatedTotal = useMemo(() => {
    ...
    return Number(selectedPlan.average_rate || 0) * Number(selectedPlan.nights || 0);
  }, [selectedPlan]);
  ...
  async function handleAvailabilitySearch(event) {
    ...
    try {
      ...
      const response = await fetchJson(`/search/availability?${params.toString()}`);
      const plans = Array.isArray(response?.available_rate_plans)
        ? response.available_rate_plans
        : [];
  ...
  return (
    <PmsShell
      ...
      sidebarMetricValue={`${availablePlans.length}`}
      sidebarMetricProgress={Math.max(16, Math.min(100, availablePlans.length * 20))}
    >
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800/70">
                ...
                <p className="mt-2 text-base font-semibold text-slate-900 dark:text-slate-100">
                  {selectedPlan ? selectedPlan.title : "No room selected"}
                </p>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  {selectedPlan
                    ? `${selectedPlan.room_id} • ${selectedPlan.rate_id} • ${selectedPlan.nights} nights`
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 p-4 dark:border-slate-700">
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Based on average nightly availability search pricing.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Pick one room-rate combination to send in the `rooms` array.
              </p>
            </div>
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-3">
                      <div className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-800/70">
                        ...
                        <p className="mt-2 text-base font-semibold">
                          {formatCurrency(plan.average_rate, plan.currency)}
                        </p>
                      </div>
                      <div className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-800/70">
                        <p className="text-xs font-bold uppercase tracking-widest text-slate-400">
                        ...
                        <p className="mt-2 text-base font-semibold">{plan.nights}</p>
                      </div>
                      <div className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-800/70">
                        <p className="text-xs font-bold uppercase tracking-widest text-slate-400">
                          Inventory
