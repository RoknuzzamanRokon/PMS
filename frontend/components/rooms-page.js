...
function createRoomEditorForm(room = {}) {
  return {
    ...
    tax_and_service_fee:
      room.tax_and_service_fee != null ? String(room.tax_and_service_fee) : "0",
    surcharges: room.surcharges != null ? String(room.surcharges) : "0",
    mandatory_fee:
      room.mandatory_fee != null ? String(room.mandatory_fee) : "0",
    resort_fee: room.resort_fee != null ? String(room.resort_fee) : "0",
    mandatory_tax:
      room.mandatory_tax != null ? String(room.mandatory_tax) : "0",
  };
}
...
function createRatePlanEditorForm(ratePlan = {}) {
  return {
    ...
    available_inventory:
      ratePlan.available_inventory != null
        ? String(ratePlan.available_inventory)
        : "0",
    total_inventory:
      ratePlan.total_inventory != null ? String(ratePlan.total_inventory) : "0",
    sold_inventory:
      ratePlan.sold_inventory != null ? String(ratePlan.sold_inventory) : "0",
...
const completionChecklist = [
  ...
  {
    key: "housekeeping_ready",
    label: "Housekeeping approved",
...
function getCompletionSummary(status) {
  const completedCount = completionChecklist.filter(
...
export function RoomsManagementPage({
  ...
  const [roomForm, setRoomForm] = useState(() =>
    createRoomForm(selectedPropertyId),
  );
  ...
  const [roomEditorForm, setRoomEditorForm] = useState(() =>
  ...
  const [ratePlanEditorForm, setRatePlanEditorForm] = useState(() =>
    createRatePlanEditorForm(),
  );
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        roomStatusStorageKey,
        JSON.stringify(roomLaunchStatuses),
      );
    } catch {}
    ...
    if (!roomDetail && fallbackCategory) {
      setRoomEditorForm(
        createRoomEditorForm({
          ...
          room_status:
            fallbackCategory.room_status ||
            fallbackCategory.status ||
            "PROCESSING",
  ...
  const summaryCards = useMemo(
    () => [
      [
        "Total Rooms",
        "meeting_room",
        String(data.summary.total_rooms),
        `${data.property.name} is synced from the backend property feed.`,
        "slate",
      ],
      [
        "Available",
        "check_circle",
        String(data.summary.available_inventory),
        "Available inventory across linked rate plans.",
        "emerald",
      ],
      [
        "Booked",
        "bedtime",
        String(data.summary.sold_inventory),
        "Sold inventory from current rate plan totals.",
        "blue",
      ],
      [
        "Blocked",
        "block",
        String(data.summary.blocked_inventory),
        "Derived from CTA, CTD, or stop-sell flags.",
        "amber",
      ],
      [
        "Occupancy",
        "bar_chart",
        `${data.summary.occupancy_percent}%`,
        "Calculated from sold versus total inventory.",
        "slate",
      ],
    ],
    [data],
  ...
  const activeCategory = useMemo(
    () =>
      data.categories.find((category) => category.room_id === activeRoomId) ||
      null,
  ...
  function handleSkipForNow() {
    ...
    router.push(
      `/inventory?property_id=${encodeURIComponent(selectedPropertyId)}`,
    );
  }
  ...
  function handleSaveImageDraft(event) {
    ...
    const nextDraft = {
      ...
      image_url:
        ...
        "https://images.example.com/rooms/placeholder.jpg",
  ...
  async function handlePreviewCreatedRoom() {
    ...
    if (!category) {
      setRoomSubmitError(
        `Created room ${createdRoomId} is not available in the overview yet.`,
      );
      return;
    }
    ...
    try {
      await fetchJson(`/rooms/${activeCategory.room_id}`, {
        ...
        body: JSON.stringify({
          ...
          room_name_lang:
            roomEditorForm.room_name_lang.trim() ||
            roomEditorForm.room_name.trim(),
    ...
    try {
      await fetchJson(`/rate-plans/${selectedRatePlanId}`, {
        ...
        body: JSON.stringify({
          ...
          available_inventory: Math.trunc(
            numberValue(ratePlanEditorForm.available_inventory),
          ),
          total_inventory: Math.trunc(
            numberValue(ratePlanEditorForm.total_inventory),
          ),
          sold_inventory: Math.trunc(
            numberValue(ratePlanEditorForm.sold_inventory),
          ),
      ...
      setRatePlanEditorForm(
        createRatePlanEditorForm(
          refreshedSelected || refreshedRoom.current_rate_plan || {},
        ),
      );
  ...
  return (
    <PmsShell
      ...
      sidebarMetricProgress={Math.max(
        10,
        Math.min(100, Math.round(data.summary.occupancy_percent)),
      )}
    >
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <h2 className="text-xl font-bold tracking-tight">Rooms Management</h2>
      ...
      {!hasSelectedProperty ? (
        ...
        >
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Open this page with a `property_id` to manage rooms, inventory, and
            room setup.
          </p>
        ...
        {summaryCards.map(([title, icon, value, note, tone]) => (
              ...
              >
                <div className="mb-5">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    Image upload is not supported by the current backend room
                    API, so this card keeps a dummy image URL ready for a future
                    image endpoint.
                  </p>
                </div>
          ...
          {activeManagementSection === "amenities" ? (
            ...
            >
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  This section is reserved for room amenities. We can add
          ...
          {activeManagementSection === "more-sections" ? (
            ...
            >
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  This area is kept for future room modules like policies, beds,
            ...
            <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="text-sm text-slate-500 dark:text-slate-400">
                  {loadingOverview
                    ? "Refreshing room table..."
                    : `${data.rooms.length} rows loaded`}
                </div>
                <button
                  type="button"
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  ...
                  {!data.rooms.length ? (
                    <tr>
                      ...
                      >
                        No rooms available for {selectedPropertyId} yet. Use the
                        Add Room card above to create one.
                      </td>
                    </tr>
                  ) : null}
                </tbody>
          ...
          {showCreateRoomModal ? (
            <div className="fixed inset-0 z-[95] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
              <div
                className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900"
              ...
              >
                <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-4 dark:border-slate-700">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                      Create the room directly from the rooms-management
                      workspace.
                    </p>
                  </div>
                  <button
                    ...
                    <label className="flex flex-col gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRoomForm((current) => ({
                            ...current,
                            room_name_lang: event.target.value,
                          }))
                        }
                        className="rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                      />
                    ...
                    <label className="flex flex-col gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRoomForm((current) => ({
                            ...current,
                            tax_and_service_fee: event.target.value,
                          }))
                        }
                        className="rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                      />
                    ...
                    <label className="flex flex-col gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRoomForm((current) => ({
                            ...current,
                            mandatory_fee: event.target.value,
                          }))
                        }
                        className="rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                      />
                    ...
                    <label className="flex flex-col gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRoomForm((current) => ({
                            ...current,
                            mandatory_tax: event.target.value,
                          }))
                        }
                        className="rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                      />
          ...
          {activeCategory ? (
            <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
              ...
              >
                {(() => {
                  const statusBadge = getBackendRoomStatusBadge(
                    roomEditorForm.room_status ||
                      activeCategory.room_status ||
                      activeCategory.status ||
                      "PROCESSING",
                  );
                  const invoiceRows = [
                    ["Base Rate", roomEditorForm.base_rate],
                    ["Tax & Service Fee", roomEditorForm.tax_and_service_fee],
                    ["Surcharges", roomEditorForm.surcharges],
                    ["Mandatory Fee", roomEditorForm.mandatory_fee],
                    ["Resort Fee", roomEditorForm.resort_fee],
                    ["Mandatory Tax", roomEditorForm.mandatory_tax],
                  ];
                  ...
                  return (
                    <>
                      ...
                      >
                        <div className="overflow-hidden rounded-xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-amber-50 shadow-sm dark:border-slate-700 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800">
                          <div className="flex flex-wrap items-start justify-between gap-4 border-b pb-5 border-slate-200/80 px-5 py-5 dark:border-slate-700">
                            <div>
                              ...
                              <div className="grid gap-2 text-sm text-slate-600 dark:text-slate-300">
                                {[
                                  [
                                    "Name",
                                    roomEditorForm.room_name ||
                                      activeCategory.room_name,
                                  ],
                                  [
                                    "Display Name",
                                    roomEditorForm.room_name_lang ||
                                      activeRoomDetail?.room_name_lang ||
                                      "-",
                                  ],
                                  ...
                                  <div
                                    key={label}
                                    className="grid grid-cols-[120px_16px_minmax(0,1fr)] items-start"
                                  >
                            ...
                            <div className="text-right">
                              <span
                                className={`inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] ${statusBadge.className}`}
                              >
                                {statusBadge.label}
                              </span>
                              <p className="mt-3 text-[11px] font-bold uppercase tracking-[0.24em] text-slate-400">
                                Total Amount
                              </p>
                              <p className="mt-1 text-xl font-black tracking-tight text-slate-900 dark:text-slate-100">
                        ...
                        <div className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 dark:border-slate-700 dark:bg-slate-900/60">
                          ...
                          <div className="grid gap-5 px-5 pt-2 pb-5">
                            {/* <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white/90 dark:border-slate-700 dark:bg-slate-900/60"> */}
                            <div className="grid grid-cols-[1fr_auto] bg-slate-100 px-4 py-3 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                              <span>Item</span>
                              <span>Amount</span>
                            </div>
                            <div className="divide-y divide-slate-100 dark:divide-slate-800">
                          ...
                          Room Status
                            ...
                            {/* <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900/70">
                          <div>
                          ...
                          >
                            ...
                            <label className="mt-4 flex flex-col gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                              ...
                              <select
                                ...
                                onChange={(event) =>
                                  setRoomEditorForm((current) => ({
                                    ...current,
                                    room_status: event.target.value,
                                  }))
                                }
                                className="rounded-2xl border border-primary/20 bg-white px-4 py-3 font-semibold text-slate-900 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:bg-slate-900 dark:text-slate-100"
                              >
                                {[
                                  "PROCESSING",
                                  "LIVE",
                            ...
                            <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 dark:border-amber-500/30 dark:bg-amber-500/10">
                              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-amber-700 dark:text-amber-300">
                                Total Amount
                              </p>
                              <p className="mt-2 text-xl font-black tracking-tight text-slate-900 dark:text-slate-100">
                                {formatCurrency(totalAmount)}
                              </p>
                            </div>
                          </div>
          ...
          {activeCategory && showEditInfoModal ? (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
              <div
                className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900"
                style={{ background: "var(--popup-card-bg)" }}
              >
                <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-4 dark:border-slate-700">
                  <div>
